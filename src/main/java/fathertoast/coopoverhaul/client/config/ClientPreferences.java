...
public class ClientPreferences extends AbstractConfigFile {
    ...
    public static class Inspection extends AbstractConfigCategory<ClientPreferences> {
        ...
        Inspection( ClientPreferences parent ) {
            ...
            jadeMode = SPEC.define( new EnumField<>( "jade_integration", COJadePlugin.Mode.OVERRIDE,
                    "Setting for the 'inspect' feature's Jade integration.",
                    " * " + TomlHelper.toLiteral( COJadePlugin.Mode.OFF ) + " - Disables Jade integration.",
                    " * " + TomlHelper.toLiteral( COJadePlugin.Mode.BACKUP ) + " - Jade's tooltip will be the " +
                            "inspect target only when Jade does not already have a tooltip to display.",
                    " * " + TomlHelper.toLiteral( COJadePlugin.Mode.OVERRIDE ) + " - While inspect is active, " +
                            "Jade's tooltip will always be the inspect target.",
                    " * " + TomlHelper.toLiteral( COJadePlugin.Mode.REPLACE ) + " - Forces Jade's tooltip to " +
    ...
    public static class HighlightSettings extends AbstractConfigCategory<ClientPreferences> {
        ...
        HighlightSettings( ClientPreferences parent ) {
            ...
            defaultSound = SPEC.define( new StringField( "global_default.sound", CoOpOverhaulObjects.SoundEvents.PING_BINK.getId().toString(),
            ...
            builder.putTag( Tags.EntityTypes.BOSSES, new HighlightEffects( 0xFF0000, CoOpOverhaulObjects.SoundEvents.PING_BOSS_LOUD ) );
            
            for( EntityType<?> entityType : ForgeRegistries.ENTITY_TYPES.getValues() ) {
                if( entityType == EntityType.PLAYER ) {
                    // Special color for players
                    builder.put( entityType, new HighlightEffects( 0x00FFFF, CoOpOverhaulObjects.SoundEvents.PING_BINK ) );
                    continue;
                }
                ...
                switch( entityType.getCategory() ) { // TODO update to use extends super in higher Crust ver, maybe also add auto-color options
                    ...
                    case MISC ->
                            builder.put( entityType, new HighlightEffects( 0xFFFF00, CoOpOverhaulObjects.SoundEvents.PING_BINK ) );
                    default ->
                            builder.put( entityType, new HighlightEffects( 0x00FF00, CoOpOverhaulObjects.SoundEvents.PING_BINK ) );
                }
            }
            entityEffects = SPEC.define( new EntityMapField<>( "entities",
                    builder.buildWithDefault( new HighlightEffects( 0xFF0000, CoOpOverhaulObjects.SoundEvents.PING_BINK ) ),
            ...
            blockEffects = SPEC.define( new BlockStateMapField<>( "blocks",
                    new BlockStateMap.Builder<>( HighlightEffects.CODEC )
                            .put( Blocks.FIRE, BlockStatePropertyMap.EMPTY, new HighlightEffects( 0xFF0000, CoOpOverhaulObjects.SoundEvents.PING_BINK ) )
                            .put( Blocks.SOUL_FIRE, BlockStatePropertyMap.EMPTY, new HighlightEffects( 0xFF0000, CoOpOverhaulObjects.SoundEvents.PING_BINK ) )
                            .put( Blocks.SCULK_SHRIEKER, BlockStatePropertyMap.EMPTY, new HighlightEffects( 0xFF0000, CoOpOverhaulObjects.SoundEvents.PING_BINK ) )
                            .buildWithDefault( new HighlightEffects( 0x00FFFF, CoOpOverhaulObjects.SoundEvents.PING_BINK ) ),
